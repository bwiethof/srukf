...
Eigen::MatrixXd MeasurementPrediction::predict(
    const Eigen::Ref<const Eigen::MatrixXd> &sigmaPoints) {

  const auto dim = dimension();
  if (dim == 0)
    return {};

  GammaPoints gammaPoints = GammaPoints::Zero(dimension(), sigmaPoints.cols());
...
long MeasurementPrediction::dimension() const {
  return std::accumulate(
      _availableSensors.begin(), _availableSensors.end(), 0L,
      [](long dim, const std::shared_ptr<const Sensor> &sensor) {
...
Eigen::MatrixXd StateTransition::transform(const Eigen::MatrixXd &sigmaPoints) {
  ...
  Eigen::MatrixXd transformedSigmaPoints(sigmaPoints.rows(),
                                         sigmaPoints.cols());
