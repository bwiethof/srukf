...
class State<ukf::core::StateFields<State_Fields...>, MapFields<Map_Fields...>>
 ...
 public:
  ...
  explicit State(const Eigen::MatrixBase<OtherDerived> &other) : Base(other) {}
  ...
  Eigen::VectorXf f(float dt, Inputs &&...inputs) const {
    const auto baseState = Base::f(dt, inputs...);
    const auto mapState = _mapFields.template apply(*this, dt);
    return baseState;
  }
  ...
  int remove(FieldIds... ids) {
    const auto fields =
        _mapFields.template getFields<Field>(std::forward(ids...));
    int count = 0;
  ...
  Eigen::MatrixXf systemNoise() const { return _systemNoise; }
 ...
 private:
  ...
  void addToState(std::size_t index, DataField &&fieldData) {
    this->template segment<Field::Size>(index) =
  ...
  void removeFromSystemNoise(std::size_t indexToRemove) {
    const auto shiftSize = this->size() - indexToRemove - Field::Size;
    const auto existingPos = indexToRemove + Field::Size;
    _systemNoise.block(indexToRemove, indexToRemove, shiftSize, shiftSize) =
        _systemNoise.block(existingPos, shiftSize);
  }

  template <typename Field>
  void addToSystemNoise(std::size_t index) {
    _systemNoise.block<Field::Size, Field::Size>(index, index) =
        Field::model.noising();
  }
