...
namespace {
    ...
    struct SensorModelSize2 : public ukf::core::SensorModel<2, MockDataSize2> {
        ...
        Eigen::Vector<float, 2UL> predict() const override {
            return {};
        }
    ...
    struct SensorModelSize4 : public ukf::core::SensorModel<4, MockDataSize4> {
        ...
        Eigen::Vector<float, 4UL> predict() const override {
            return {};
        }
...
TEST(SensorData, construction) {
    ...
    // only contains first sensor
    {
        ...
        const Eigen::Matrix2f expectedMatrix = (Eigen::Matrix2f() << 2.0, 0, 0, 2.0).finished();

        ASSERT_TRUE(sensorData.vector().isApprox(expectedVector));
        ASSERT_TRUE(sensorData.noising().isApprox(expectedMatrix));
    }

    sensorData.setMeasurement<SensorSize4>({.value_1=2.1f, .value_2=2.2f, .value_3=2.3f, .value_4=2.4f,});
