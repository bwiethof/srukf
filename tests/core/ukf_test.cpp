...
namespace {
    ...
    namespace sensortest {
        ...
        struct SensorModelSize2 : public ukf::core::SensorModel<2, MockDataSize2, statetest::MockField> {
            Eigen::Matrix<float, 2UL, 2UL> noising() const override {
                return Eigen::DiagonalMatrix<float, 2>(2, 2).toDenseMatrix();
            }

            Eigen::Vector<float, 2UL> predict(const statetest::MockField &field) const override {
        ...
        struct SensorModelSize4 : public ukf::core::SensorModel<4, MockDataSize4, statetest::MockFieldSize5> {
            Eigen::Matrix<float, 4UL, 4UL> noising() const override {

                return Eigen::DiagonalMatrix<float, 4>(4, 4, 4, 4).toDenseMatrix();
            }

            Eigen::Vector<float, 4UL> predict(const statetest::MockFieldSize5 &field) const override {
        ...
        using SensorSize2 = ukf::core::Field<2, SensorModelSize2>;
        using SensorSize4 = ukf::core::Field<4, SensorModelSize4>;
...
TEST(Ukf, Initialization_test) {
    ...
    const auto checkValues = [](const ukf::core::Ukf<statetest::Fields> &ukf) {
        ...
        const Eigen::Vector<float, 8> expectedState{1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f};
    ...
    {
        ukf::core::State<statetest::Fields> state = Eigen::Vector<float, 8>{1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f};
    ...
    // initialize via move State
    {
        ukf::core::State<statetest::Fields> state = Eigen::Vector<float, 8>{1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f};
        ukf::core::Covariance<statetest::Fields> covariance = prepareCovariance();;
        ukf::core::Ukf<statetest::Fields> testUkf(std::move(state), std::move(covariance));

        checkValues(testUkf);
    }
    // initialize via reference matrix
    {
        Eigen::Vector<float, 8> state = Eigen::Vector<float, 8>{1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f};
...
TEST(Ukf, Stepping_test) {
    ...
    const ukf::core::State<statetest::Fields> initialState = Eigen::Vector<float, 8>{1.0f, 1.1f, 1.2f, 1.3f, 1.4f, 1.5f,
                                                                                     1.7f, 1.8f};
    const ukf::core::Covariance<statetest::Fields> initialCovariance = Eigen::Matrix<float, 8, 8>::Identity();
    ...
    sensorData.setMeasurement<sensortest::SensorSize2>({.value_1=1.f, .value_2=2.f});
    double dt = 1.0;


    const auto [expectedState, expectedCovariance] = refUkf.timeStep(initialState, initialCovariance, dt, sensorData);
