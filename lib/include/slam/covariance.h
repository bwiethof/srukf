...
namespace ukf {
    namespace slam {
        ...
        class Covariance<ukf::core::StateFields<State_Fields...>, MapFields<Map_Fields...>>
        ...
        public:
            ...
            tl::optional<Eigen::Matrix<float, FieldType::Size, FieldType::Size>> getFieldData(std::size_t id) const {
                const auto field = _mapFields.template get<FieldType>(id);
                if (!field) {
                    return tl::nullopt;
                }

                return this->block<FieldType::Size, FieldType::Size>(field->offset, field->offset);
            }
            ...
            void remove(FieldIds ...ids) {
                ...
                for (const auto &id: idContainer) {
                    const auto it = std::binary_search(ref.begin(), ref.end(), id); // is a bool so should not work
                    if (it == ref.end()) {
                        continue;
                    }
        ...
        private:
            ...
            void addToState(std::size_t index, DataField &&fieldData) {
                this->segment<Field::Size>(index) = Field::model.toState(std::forward<DataField>(fieldData));
