...
struct StateDependencies : core::StateDependencies<T...> {};

template <std::size_t N, typename Measurement, typename Self,
          typename... Inputs>
struct Model : public core::HasModel<N, core::StateDependencies<Inputs...>> {
  // Shall implement transformation from measurement to representation
  virtual Eigen::Vector<float, N> toState(
      const Measurement &measurement) const = 0;

  virtual Eigen::Matrix<float, N, N> toCovariance(
      const Measurement &measurement) const = 0;
  ...
  virtual Eigen::Vector<float, N> mapUpdate(float, const Self &,
...
struct Field<N, StateDependencies<Dependencies...>, core::Inputs<Input...>>
    : Field<N, core::StateDependencies<Dependencies...>,
            core::Inputs<Input...>> {
  Field(std::size_t offset, std::size_t id)
      : core::Field<N, StateDependencies<Dependencies...>,
                    core::Inputs<Input...>>(offset),
        _id(id) {}
...
class MapFields {
 ...
 public:
  ...
  std::vector<Field> getFields(FieldIds... ids) const {
    ...
    const auto it =
        std::copy_if(_fields.begin(), _fields.end(), fields.begin(),
                     [&idContainer](const Field &f) {
                       return std::find(idContainer.begin(), idContainer.end(),
                                        f._id) != idContainer.end();
                     });
  ...
  bool remove(std::size_t id) {
    ...
    const auto it =
        std::remove_if(fields.begin(), fields.end(), IdMatcher<Field>{id});
    if (it == fields.end()) {
      return false;
    }
